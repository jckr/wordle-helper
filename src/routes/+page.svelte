...
function update() {
	validSolutions = solutions.filter(word => {
		const letters = word.split('');
		// a word is valud if:
		return letters.every((letter, i) => (
			// every placed letter is at the same place in this word
			(placedValues[i] === '' || placedValues[i] === letter) &&
			// no letter in this word is part of the "absent" list 
			(!absent.includes(letter)) &&
			// if a letter is unplaced at this position, the word can't
			// have the same letter at that very position
			(!unplacedValues[i].includes(letter))
		)) && 
			// every unplaced letter is still present in the word.
			unplacedValues.every(unplaced => 
				unplaced.split('').every(letter => word.includes(letter))
			);
	})
}
...
<div class="letter-block">
	...
	{#each [0,1,2,3,4] as index} 
		<div class="column">
		<div class="unplaced-letters">
		{unplacedValues[index]}
		</div>
		<div 
